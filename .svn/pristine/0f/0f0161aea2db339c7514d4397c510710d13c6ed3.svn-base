...
function drawPopup1(result) {
	...
	for (let i=0; i<spdList.length; i++){
		lanenoTh += '<th>'+spdList[i].lane_no+'차로</th>';
		slevelTd += '<td>'+spdList[i].slevel+'유형</td>';
		noteTd += '<td>'+spdList[i].slevel_note+'</td>';
		...
		spdTd += '<td>'+spd_value+'</td>';
		tmslTd += '<td>'+spdList[i].tmsl_note+'</td>'; //추후수정
	}
